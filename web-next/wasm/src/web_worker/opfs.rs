...
use web_sys::{FileSystemDirectoryHandle, FileSystemFileHandle, FileSystemGetDirectoryOptions, FileSystemGetFileOptions, FileSystemReadWriteOptions, FileSystemSyncAccessHandle};
    ...
    fn received(&mut self, scope: &WorkerScope<Self>, msg: Self::Input, id: HandlerId) {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            ...
            let result = match msg.message {
                ...
                OpfsOperation::Write(data, position) => {
                ...
                OpfsOperation::Read(size, position) => {
                    match file_guard.as_ref() {
                        Some(file_handle) => {
                            let options = FileSystemReadWriteOptions::new();
                            options.set_at(position as f64);
                            ...
                            match file_handle.read_with_js_u8_array_and_options(&buffer, &options) {
                                Ok(s) => {
                                    OpfsOperationOutput::Binary(buffer.subarray(0, s as u32))
                                },
                                Err(e) => OpfsOperationOutput::Error(e),
                            }
                        }
