...
impl RelayRtcClient {
    pub async fn accept_offer(sdp_offer: &str, channels: Vec<DataChannel>) -> Result<(Box<Self>, String), RelayRtcError> {
    ...
    fn log_upload_limit_applied(
        ...
        if !self.upload_limit.limit_active_logged {
            ...
            log::info!(
                "[relay-rtc] upload_limit active branch={} queue={}B other_down={:.0}B/s measured={:.0}B/s current={:.0}B/s applied={:.0}B/s",
                branch,
                queued_bytes,
                other_side_download_bps,
                measured_upload_bps,
