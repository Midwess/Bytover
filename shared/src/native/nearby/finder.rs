...
impl NearbyFinder {
    pub fn new(public_port: u16) -> Self {
        Self { daemon: ServiceDaemon::new().expect("Failed to create mDNS service"), public_port }
    }

    pub async fn start(&self) {
        let daemon = self.daemon.clone();
        let receiver = spawn_blocking(move || {
            daemon.browse(SERVICE_TYPE).expect("Failed to browse")
        ...
        }).await.expect("Failed to spawn blocking for register service");
    }
