    ...
    pub async fn handle(&self, request_id: u32, effect: TransferOperation) -> CoreOperationOutput {
        match effect {
            TransferOperation::SendSession(session) => {
                let Some(connection) = self.web_rtc.get_connection(session.peer_id().unwrap_or_default()).await.ok().and_then(|connection| connection.upgrade()) else {
            ...
            TransferOperation::SendResource(peer_id, resource) => {
                let Some(connection) = self.web_rtc.get_connection(peer_id).await.ok().and_then(|connection| connection.upgrade()) else {
                    return CoreOperationOutput::ConnectionError(ConnectionWebRtcErrors::ConnectionNotFound.into());
                };

                let resource_order_id = resource.order_id;
                let result = connection.send_resource(request_id, resource).await;
                ...
                match result {
                    ...
                    Err(error) => {
                        CoreOperationOutput::Transfer(TransferOperationOutput::TransferResourceProgressUpdate(
                            TransferProgress::fail(resource_order_id, format!("{:?}", error))
                        ))
                    }
                }
            }
            TransferOperation::AnswerSessionRequest(peer_id, resources, peer_request_id, response) => {
                ...
                let result = connection.answer_session_request(request_id, resources, peer_request_id, response).await;
