...
async fn test_false_retransmission_with_latency() {
    ...
    println!(
        ...
        frames_received, frames_lost, retransmit_count, false_retransmit
    );
    println!(
    ...
    println!("Observed loss rate: {:.2}%", receiver_loss_rate * 100.0);
    ...
    if retransmit_count > 0 && false_retransmit > 0 {
        ...
        println!(
            "SUCCESS: Reproduced false retransmission - {:.1}% of retransmits were unnecessary \
             (original packet was delayed, not lost)",
            false_ratio * 100.0
        );
    }
}
...
async fn test_threshold_with_multiple_configs() {
    ...
    let configs = [
        TestConfig {
            delay_rate: 0.10,
            rtt_ms: 20,
            delay_iterations: 100,
            name: "Low delay (10%), Low RTT (20ms)"
        },
        TestConfig {
            delay_rate: 0.20,
            rtt_ms: 30,
            delay_iterations: 150,
            name: "Medium delay (20%), Low RTT (30ms)"
        },
        TestConfig {
            delay_rate: 0.15,
            rtt_ms: 50,
            delay_iterations: 200,
            name: "Medium delay (15%), Medium RTT (50ms)"
        },
        TestConfig {
            delay_rate: 0.25,
            rtt_ms: 100,
            delay_iterations: 300,
            name: "High delay (25%), High RTT (100ms)"
        },
        TestConfig {
            delay_rate: 0.30,
            rtt_ms: 30,
            delay_iterations: 400,
            name: "High delay (30%), Low RTT (30ms)"
        }
    ];
    ...
    for (i, config) in configs.iter().enumerate() {
        ...
        let mut network = NetworkSimulatorWithDelay::new(config.delay_rate, config.delay_iterations, (i as u64) * 1000 + 42);
        let mut retransmit_packets_sent: u64 = 0;
        let mut received_data: Vec<u8> = Vec::with_capacity(data_size);
