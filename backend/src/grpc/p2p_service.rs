...
use schema::devlog::bitbridge::{
    ...
    GenPeerRequest, GenPeerResponse, GetDeviceAliasesRequest, GetDeviceAliasesResponse, P2pSession, PeerMessage,
};
...
impl P2pOrchestrationService for P2PGrpcService {
    ...
    async fn gen_peer(&self, request: Request<GenPeerRequest>) -> Result<Response<GenPeerResponse>, Status> {
        ...
        let (name, avatar_url, email, signalling_id) = match user {
            Some(u) => {
                let sig = format!("{}_{}_{}", devlog_sdk::distributed_id::gen_id().await, u.order_id, device.device_unique_key);
                (Some(u.display_name.clone()), u.avatar_url.clone().unwrap_or_default(), Some(u.email.clone()), Some(sig))
            }
            None => {
