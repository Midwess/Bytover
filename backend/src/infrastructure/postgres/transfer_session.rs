...
fn compute_status(session: &TransferSession) -> TransferSessionStatus {
    ...
    let has_non_canceled_failed = progresses
        .iter()
        .any(|p| matches!(p.status(), TransferProgressStatus::Failed(msg) if msg != "Canceled"));
    if has_non_canceled_failed {
        return TransferSessionStatus::Failed;
    }

    let has_canceled = progresses
        .iter()
        .any(|p| matches!(p.status(), TransferProgressStatus::Failed(msg) if msg == "Canceled"));
    if has_canceled {
...
impl TransferSessionRepository for TransferSessionPostgresRepository {
    ...
    async fn delete_stale_sessions(&self, cloud_storage: &dyn CloudStorage) -> Result<(), RepositoryError> {
        ...
        for row in rows {
            ...
            let resources_json: Option<serde_json::Value> =
                row.try_get("", "resources").map_err(|e| RepositoryError::DbError(e.to_string()))?;
        ...
        if !session_ids_to_delete.is_empty() {
            let placeholders: Vec<String> = session_ids_to_delete.iter().enumerate().map(|(i, _)| format!("${}", i + 1)).collect();
            let delete_sql = format!("DELETE FROM transfer_session WHERE id IN ({})", placeholders.join(", "));
