...
use crate::webrtc::signalling::{SignalingClient, SignallingError};
...
fn map_signalling_error(error: SignallingError) -> CoreError {
    match error {
        SignallingError::Unauthorized(message) => CoreError::Unauthorized(message),
        SignallingError::BadRequest(message) => CoreError::BadRequest(message),
